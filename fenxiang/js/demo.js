$(function(){
  ...
  var img_id = url[0].substring(url[0].indexOf("=")+1);

  var user_id = url[1].substring(url[1].indexOf("=")+1);
...
//   }
//   if(GetQueryString("txtval")!=null && GetQueryString("txtval").length>-1){
//       txtval=GetQueryString("txtval");
//       alert(txtval);
//     }
...
//点击进入下载
 ...
 //渲染头部数据
    ...
    $(".txt").focus(function(){
        $(this).css('position','fixed','bottom','1rem');
        $('.send').css('position','fixed','bottom','1rem');
    })
    $(".txt").blur(function(){
    ...
    $(".main").on("click",".send",function(){
      ...
      if(!code){
         ...
         if(txtval!=""){
            ...
            window.location.href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5bd2876a775d0525&redirect_uri=http%3a%2f%2fwww.meimei.yihaoss.top%2ffenxiang%2fpostbardetial.html?img_id="+img_id+"&txtval="+txtval+"&response_type=code&scope=snsapi_userinfo&state=111";
        ...
        if(window.localStorage.getItem("login_user_id")!=null && $(".txt").val() != ""){
           $.ajax({
                          url:'http://api.baobaoshowshow.com/index.php?r=BabyShowV1/PublicListingReview',
                          data:{
        ...
        }else if(window.localStorage.getItem("login_user_id")!=null && $(".txt").val() == ""){
          alert("评论不能为空");
          $(this).disable();
