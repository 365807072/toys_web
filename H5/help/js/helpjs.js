	...
	function getUserActivityInfo(help_user_id){
		...
		success:function(data){
				...
				for(var i in listData){
					footHtml+='<p class="userList" colorindex="'+listData[i].user_id+'"><span><img src="'+listData[i].avatar+'"></span><span>'+listData[i].nick_name+'</span><span>'+listData[i].help_name+'</span></p>';

				}
				$(".foot").html(footHtml);

				// $(".foot .userList").each(function(){
				// 	color=$(this).attr("colorindex");
				// 	if(color==help_user_id){
				// 		$(this).css("background","#fd6363","opacity",".8")
				// 	}
				// })
				if(res.success==true){
					//var height="<%=res.data1.help_number %>";
           			...
           			var aa=parseInt((clientWidth/10)*(res.data1.help_number/100)*4)+"%";
           			console.log(aa);
           			if(res.data1.help_number==100){
           				alert("助力值已满100%！");
           			}
           			...
           			// var peoNumHtml="";
           			// peoNumHtml+='(已有<span class="peoNumed">'+res.data1.help_number+'</span>个助力值)';
