...
function getListMessage(){
	$.ajax({
		...
		success:function(data){
			...
			request();
			box1Html +='<span>问题用户</span><input type="text" name="" class="user" value='+listData.user_id+'>';
			$(".box1").html(box1Html)

			box6Html +='<span>问题订单ID</span><input type="text" name="" class="order" value='+listData.order_id+'>';
			$(".box6").html(box6Html)

			box2Html += '<span>问题</span><textarea class="question">'+listData.problem+'</textarea>';
			$(".box2").html(box2Html);

			box3Html += '<span>处理记录</span><textarea class="result">'+listData.handle_record+'</textarea>';
			$(".box3").html(box3Html);

			box7Html += '<span>处理记录</span><textarea class="result">'+listData.handle_record+'</textarea>';
			$(".box7").html(box7Html);
...
function getIssue(){
	...
	var fileSr = fileSelect.toString();
	console.log(fileSr)
	$.ajax({
		url:'https://api.baobaoshowshow.com/index.php?r=WebBaoShow/pubCustomerPros2',
		type:'POST',
		data:{
			id:userId,
			user_id:loginUser,
...
function setImagePreviews(avalue) {
    ...
    for (var i = 0; i < fileList.length; i++) {
        dd.innerHTML += "<div style='margin-top: 10px;' > <img id='img" + i + "'  /> </div>";
        ...
        fileSelect.push(docObj.files[i]);
