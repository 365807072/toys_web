$(function(){
	...
	//会员卡的点击事件
	$(".content").on("click",".memberlist",function(event){
 	...
 	//banner的点击事件
    $(".content").on("click",".bannerlist",function(event){
    ...
    $(".content").on("click",".bban",function(event){
	...
	$(".content").on("click",".allpic",function(){
		if(phoneType==1){
			window.call.callMethodToAllPage();
		}else if(phoneType==2){
			callMethodToAllPage();
		}else{
			window.call.callMethodToAllPage();
	...
	//五图中的左边大图的点击事件
	$(".content").on("click",".left",function(event){
	...
	//点击五图的右边小图
	$(".content").on("click",".rihgtlist",function(event){
	...
	//点击三图中的一个
	$(".content").on("click",".asslist_cout",function(event){
	...
	//点击五图中的更多
	$(".content").on("click",".wmore",function(event){
	...
	//点击三图中的更多
	$(".content").on("click",".smore",function(event){
		categoryId=$(this).attr("catindex");
		// window.location.href="http://www.meimei.yihaoss.top/H5/toy_count/all.html?login_user_id="+userId+"&category_id="+categoryId
		if(phoneType==1){
			window.call.callMethodToCategory(categoryId);
	...
	function request(){
		...
		$.ajax({
			url:'https://api.baobaoshowshow.com/index.php?r=BabyShowV77/AppIndexClass&login_user_id='+userId,
			type:'GET',
			datatype:'json',
			success:function(data){
				...
				for(var i in firstData){
					if(firstData[i].show_type==1){
						mainHtml+='<div class="bban" webindex="'+firstData[i].web_link+'" typeindex="'+firstData[i].source+'" bussindex="'+firstData[i].img_id+'">';
	...
	function slide(){
		...
		$.ajax({
			url:'https://api.baobaoshowshow.com/index.php?r=BabyShowV77/AppIndexCard&login_user_id='+userId,
			datatype:'json',
			type:'GET',
	...
	function banner(){
		...
		$.ajax({
			url:'https://api.baobaoshowshow.com/index.php?r=BabyShowV77/AppIndexBanner&login_user_id='+userId,
			type:'GET',
			datatype:'JSON',
			success:function(data){
				var data=JSON.parse(data);
