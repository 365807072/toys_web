$(function(){
	...
	$('.main').dropload({
	    ...
	    loadDownFn : function(me){
	    	$.ajax({
				url:'http://api.baobaoshowshow.com/index.php?r=BabyShowV1/getToysCategoryList&login_user_id='+login_user_id+'&is_order=0&post_create_time='+postTime,
				type:'GET',
				datatype:'josn',
				success:function(data){
					Data=JSON.parse(data);
	...
	$(".content").on("click",".box",function(){
		...
		window.location.href="http://www.meimei.yihaoss.top/fenxiang/toy_cout.html?business_id="+businessId+"&login_user_id="+login_user_id+"&title=邀请";
	})

	//右上角的邀请页面
	$(".right").on("click",function(){
		window.location.href="http://mp.weixin.qq.com/s/3SowJnlWO_aBFQkXm-nE9Q";
	})
})