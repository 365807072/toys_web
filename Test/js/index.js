		...
		callback:function(data){
			if(issearch==false){
				...
				if(data.length!=0){	
					...
					//console.log(lastpost_create_time);
					for(var i in data){
...
/*if(confirm("是否让舜鑫国际旅游（北京）有限公司租赁使用您当前的位置")){
	isPosition = true;
	//获取地理位置
	isPositionFun();
...
$('.nav ul li').on("tap",function(event){
	...
	// 		screenhtml = "";
	// 		htmlContent();
	// 		time = data[data.length-1].img.post_create_time;
	// 		for(var i in data){
	// 			var scrData=data[i].img;
	// 			screenhtml+="<li class='center' id="+scrData.business_id+">";
	// 			screenhtml+="<dl><dt><img src="+scrData.business_pic+"></dt><dd><p><span>"+
...
$(".l_content").on("click",".center",function(){
	...
	window.location.href="http://www.meimei.yihaoss.top/Test/business_details.html?login_user_id="+login_user_id+"&business_id="+business_id;
});
$("footer ul").on("tap",".study",function(){

	window.location.href="http://www.meimei.yihaoss.top/Test/business_list.html?login_user_id="+login_user_id;
});	
$("footer ul").on("tap",".order",function(){

	window.location.href="http://www.meimei.yihaoss.top/Test/order.html?login_user_id="+login_user_id+"&open_id="+openid;
});
function downLoad(){
    ...
    if(isWeiXin()){
    	window.location.href="http://www.meimei.yihaoss.top/download.php?from=singlemessage&isappinstalled=0"
    }else{
    	return false;
    }
    if(isAndroid){
