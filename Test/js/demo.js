...
function createXmlHttpRequest(){    
    ...
    if(!code){
    	href = "https://open.weixin.qq.com/connect/oauth2/authorize?appid=wx5bd2876a775d0525&redirect_uri=http%3a%2f%2fwww.meimei.yihaoss.top%2fTest%2fbusiness_details.html&response_type=code&scope=snsapi_userinfo&state="+login_user_id+"="+business_id;
    	
    }else{
    	href = "javascript:;";
    	login_user_id = GetRequest().state.split('%3D')[0];
    	business_id = GetRequest().state.split('%3D')[1];
    	$.get('http://www.meimei.yihaoss.top/Test/php.php?code='+code+'&state=111',function(data){
    				...
    				$(".onorder").on("click",function(){
						if(storage.getItem("login_type")==2){
							...
							if(order_role=0){
								...
								var PayUrl = "http://www.meimei.yihaoss.top/Test/pay.html?login_user_id="+login_userId+"&business_id="+business_id+"&package="+data_index+"&order_rol=0&code="+code+"&open_id="+openid;
								//alert(PayUrl);
...
//如果是商家 不可以点击“立即抢购” 用户的话登陆的直接进入支付 没有登陆去登陆然后跳入支付 
	...
	function reload(s){
		$.ajax({
			...
			success:function(data){
				...
				if(result.user_name!=""){
					userhtml="<p>秀秀用户评论</p><p><span>"+result.user_name+"</span><span><img src="+result.grade3+"></span><span>"+result.user_time+"</span></p><p>"+result.user_msg+"</p>";
					$(".userPingJia").html(userhtml); 
				}else{
					$(".userPingJia").hide();
				}
	...
	$(".l_btn").on("click",function(){
		window.location.href="http://www.meimei.yihaoss.top/Test/business_list.html";
	})
